...
- Code and reported runs: this repository, release tag `sc26-camera-ready`.
  - `surrogate_models/phase1_lstm_mr/` … `phase5_grouped_mr/`: the five reported runs (configuration, data split, training history, per-output metrics, run log and report). The per-output-type results of the paper come from their `metrics.csv` and `phase_summary.json`.
